...
/// Defines the [FCStepper]'s main axis.
enum StepperType {
...
///
/// Used by [FCStepper.controlsBuilder].
@immutable
class ControlsDetails {
  ...
  /// Index that is active for the surrounding [FCStepper] widget. This may be
  ...
  /// just activated the transition to a new step, some [FCStepper.type] values will
...
/// A material step used in [FCStepper]. The step can have a title and subtitle,
...
@immutable
class Step {
  /// Creates a step for a [FCStepper].
  const Step({
    required this.title,
    this.subtitle,
...
class FCStepper extends StatefulWidget {
  ...
  @override
  State<FCStepper> createState() => _FCStepperState();
}

class _FCStepperState extends State<FCStepper> with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(FCStepper oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    assert(() {
      if (context.findAncestorWidgetOfExactType<FCStepper>() != null) {
        throw FlutterError(
